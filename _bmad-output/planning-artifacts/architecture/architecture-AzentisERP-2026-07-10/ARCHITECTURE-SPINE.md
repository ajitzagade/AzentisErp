...
- **Rule:** One Tenant Settings DocType (Single, one record per site) holds all tenant branding fields and `enabled_modules` (Table MultiSelect field, rows referencing canonical Module Def names — see Consistency Conventions). Every read path (branding injection, sidebar filtering) goes through one accessor function using `frappe.get_cached_doc("Tenant Settings")` — never a scattered raw `frappe.get_single`/`frappe.get_doc` call at each call site. Frappe invalidates this cache automatically on save, so the single accessor is never stale — this is the same mechanism AD-3 relies on for live updates, not a competing one.
