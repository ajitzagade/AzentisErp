...
1. Given GitHub org access, when ERPNext is forked under our org, then `github.com/ajitzagade/erpnext` exists as our controlled copy, on the `version-15` branch. [Source: prd.md §10.7]
...
- [ ] Task 1: Fork ERPNext under the org's GitHub account (AC: 1)
  - [ ] Fork `https://github.com/frappe/erpnext` to `https://github.com/ajitzagade/erpnext` — this is a **separate GitHub repo from `ajitzagade/AzentisErp`** (which tracks this project's own planning/implementation artifacts). `ajitzagade/erpnext` is a real GitHub *fork* of `frappe/erpnext` (preserving history/lineage, not a fresh empty repo) — use GitHub's Fork feature or `gh repo fork frappe/erpnext --org ajitzagade`, not `git init`.
...
- [ ] Task 4: Pull the forked ERPNext into the bench (AC: 2)
  - [ ] `bench get-app erpnext --branch version-15 https://github.com/ajitzagade/erpnext` (verify exact flag order against the installed `bench` CLI's own `--help` — minor syntax differences exist across bench versions)
...
- **GitHub org resolved: `ajitzagade`** (personal account). The PRD and Architecture Spine still say `OUR_ORG` as a stand-in — treat `ajitzagade` as that value wherever `OUR_ORG` appears in those source documents.
- `<bench-name>` and `<site-name>` in Tasks 3/5 are lower-stakes placeholders (local dev naming, not a public/legal identifier like `ajitzagade`) — use `docs/product.md`'s original suggestion if no preference is given: bench name `myerp`, site name `dev.local`.
