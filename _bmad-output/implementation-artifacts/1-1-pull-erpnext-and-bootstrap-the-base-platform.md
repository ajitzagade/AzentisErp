...
- [x] Task 4: Create a site and install ERPNext (AC: 2)
...
- [x] Task 5: Configure `ajitzagade/AzentisErp` to track only our own code (AC: 3)
  - [x] Switched to a **whitelist pattern** rather than an explicit exclusion list: `myerp/*` / `!myerp/apps` / `myerp/apps/*` / `!myerp/apps/our_brand`. An explicit list (as originally planned) proved unsafe in practice — it missed `sites/dev.local/site_config.json` (contains a real auto-generated DB password), per-site logs, and a stray screenshot file that landed in the bench root. **Scope note:** this is intentionally broader than AC3's literal text, which only names `apps/frappe`/`apps/erpnext` — the whole bench root needed covering once real sensitive content (the DB password, per-site logs) turned up outside those two specific paths, so the rule exceeds AC3 in the direction of more safety, not less compliance. At the `myerp/` boundary the whitelist reliably excludes everything not explicitly allowed back in; it does *not* guarantee safety *inside* the allowed `our_brand` zone, which still leans on unrelated generic `.gitignore` rules (`*.env`, `*.log`, `__pycache__/`) — see the deferred finding for Story 1.2.
...
- [x] [Review][Decision] Real credentials committed in plaintext to git history — the MariaDB root password (`frappe_dev_root`) and ERPNext Administrator password (`admin`) are written directly into this story file's Task 4 subtask text and Completion Notes #4/#6, now part of commit `2f9a53d`. **Resolved:** left as-is — local-dev-only credentials (MariaDB accepts localhost connections only), private repo, low real-world risk. Rotate if that risk profile ever changes (e.g., repo made public, or server exposed beyond localhost).
- [x] [Review][Patch] Task 4's parent checkbox left unchecked despite all subtasks being `[x]` [1-1-pull-erpnext-and-bootstrap-the-base-platform.md:37]
- [x] [Review][Patch] Completion Note #10 overclaims "All 4 ACs verified" unconditionally, when AC2's literal text (which enumerates HR among required modules) is not fully met — HR is documented as missing in Completion Note #9, but the summary doesn't carry that caveat [Completion Notes #9, #10]
- [x] [Review][Patch] AC2's "setup wizard completed / modules load with zero errors" claim is true (independently verified by the Acceptance Auditor against the screenshot and `bench_start.log`) but is not cited to any evidence path in the story document — the screenshot file exists on disk but is never referenced, and the debug log section explicitly says nothing is persisted [Dev Agent Record — Debug Log References, Completion Notes]
- [x] [Review][Patch] No documented guard against a manually-started Redis process on ports 11000/13000 persisting and causing the same connection-refused failure the story had to debug, if `bench start` is ever run again without those ports being free first [Dev Notes]
- [x] [Review][Patch] `.gitignore`'s whitelist scope (entire `myerp/*` tree) is broader than AC3's literal text (which only names `apps/frappe`/`apps/erpnext`) — the broader scope is the right call and already has rationale in Completion Note #8, but Task 5 doesn't explicitly reconcile the scope against AC3's narrower wording [Task 5, .gitignore]
- [x] [Review][Defer] Whitelist gitignore inside `our_brand` relies on unrelated global patterns (`*.env`, `*.log`, `__pycache__/`) for safety rather than a dedicated rule, and `core.ignorecase=true` (macOS default) means a case-variant directory name could theoretically get silently whitelisted — deferred, real but only actionable once `our_brand` exists (Story 1.2); not a defect in this story
- [x] [Review][Defer] `apps/erpnext`'s shallow clone (`--depth 1`) — the claim that future `git fetch`/checkout to a newer pinned commit "still works" is asserted, not tested — deferred, real risk to Epic 5 Story 5.4's re-pin mechanism but not testable or actionable until that story is reached
...
- **Gotcha for anyone running `bench start` again:** the bench's own Redis instances (queue on port 11000, cache on port 13000) are managed entirely by `bench start`'s Procfile. Do not manually start `redis-server` on those ports for any reason (e.g., to unblock an `install-app` step) and leave it running — a manually-started instance squatting on those ports will make `bench start`'s own Redis fail to bind, which cascades to killing the entire process group (this exact failure happened once during this story; see Completion Note #5). If you hit a "Cannot connect to redis_cache"/"Connection refused" error, check `lsof -i :11000 -i :13000` for a stray process before assuming something else is wrong.
...
10. AC1, AC3, AC4 fully verified. **AC2 substantially met, with one known, deliberate exception:** (1) ERPNext pulled directly from `frappe/erpnext` via `upstream` remote, pinned to `b5f7846` (v15.115.0) — no fork exists or was created; (2) setup wizard completed and the ERPNext desk loads with all core modules **except HR** (see #9 — a real, unmet piece of AC2's literal text, not glossed over: HR is no longer part of core ERPNext as of v14+, tracked as an Epic 3 follow-up, not fixable in this story). Evidence: `myerp/Screenshot 2026-07-11 at 12.28.45 AM.png` (user-captured, shows the working desk with the module list) and `/tmp/bench_start.log` line `127.0.0.1 - - [11/Jul/2026 00:27:44] "GET /app/home HTTP/1.1" 200 -` (confirms the desk dashboard, reached post-setup-wizard, served successfully); (3) `apps/frappe`/`apps/erpnext` gitignored via the whitelist pattern, confirmed via `git add -A --dry-run` staging zero files; (4) both `apps/frappe` and `apps/erpnext` are git-clean (zero local modifications), baseline commits recorded for Epic 5's later re-pin verification.
